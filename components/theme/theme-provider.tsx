...
const ALL_THEME_VARS = [
  "--brand-bark", "--brand-sand", "--brand-stone", "--brand-cream",
...
export function ThemeProvider({
  ...
  const [savedAppearance, setSavedAppearance] = React.useState<"light" | "dark" | "auto">(initialAppearanceMode);
  const [currentTheme, setCurrentThemeState] = React.useState(initialTheme);
  const [appearanceMode, setAppearanceModeState] = React.useState<"light" | "dark" | "auto">(initialAppearanceMode);

  React.useEffect(() => {
    const localTheme = localStorage.getItem(LS_THEME);
    const localAppearance = localStorage.getItem(LS_APPEARANCE) as "light" | "dark" | "auto" | null;
  ...
  const setTheme = React.useCallback((theme: string) => setCurrentThemeState(theme), []);
  const setAppearance = React.useCallback((mode: "light" | "dark" | "auto") => setAppearanceModeState(mode), []);
